/**
 * WinnerShowcaseThemeStudio.jsx — Author Studio › Winner Showcase Theme
 * (Winner Showcase Theme Engine directive — clones the Achievement
 * Experience Engine rather than re-designing around it).
 *
 * Structurally a TWIN of AchievementExperienceStudio.jsx — same
 * CRUD/draft/publish/scheduling contract, same generic experience-configs
 * API, same admin auth, same "draft never live until Published" rule —
 * scoped to experienceType="winner_showcase_theme" instead of
 * "achievement_top_earner", and reusing achievementThemes.js's preset
 * library DIRECTLY (not a forked copy): "clone exactly" means starting
 * from the same visual quality, and a second preset file would be exactly
 * the "second theme-authoring system" achievementConfigResolver.js's own
 * header comment warns against building.
 *
 * Deliberately a SEPARATE Studio tab from WinnerShowcaseStudio.jsx (the
 * earlier Dashboard-Polish round's admin page for the winner DATA — which
 * showcases exist, unpublish, remove a winner from the display) rather
 * than merged into one file: this codebase already keeps data-admin and
 * theme-admin as separate tabs for the closely related Achievement
 * concept too (see "achievementcenter"/TrophyAchievementStudio.jsx vs
 * "achievementexp"/AchievementExperienceStudio.jsx in StudioPage.jsx) —
 * a real precedent, not an invented split.
 *
 * Live preview renders the REAL FridaySpeakingWinnersPanel (same
 * "what you see here is what ships" guarantee as Achievement's own
 * preview) — it shows the actual latest winner_showcase snapshot via its
 * own useWinnerShowcaseRotation() fetch; only the presentation layer is
 * fed by this form's themeConfig prop.
 */
import { useCallback, useEffect, useMemo, useState } from "react";
import {
  Award, Plus, RefreshCw, Trash2, Pencil, X, Eye,
...
import FridaySpeakingWinnersPanel from "../eduhub/components/dashboard/FridaySpeakingWinnersPanel";
import {
...
import { DEFAULT_NEXT_SESSION_COUNTDOWN } from "../eduhub/lib/experienceConfig/winnerShowcaseConfigResolver";
...
const BLANK = {
  ...
  appearance: {
    ...
    overrides: {},
    artwork: null,
    nextSessionCountdown: { ...DEFAULT_NEXT_SESSION_COUNTDOWN },
  },
...
function PresetSwatch({ preset, active, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      data-testid={`winnerthemeexp-preset-${preset.id}`}
      aria-pressed={active}
...
function DecorationRow({ type, config, onChange }) {
  return (
    <div className="flex items-center gap-2 mb-1.5" data-testid={`winnerthemeexp-decoration-${type}`}>
      <button
        type="button"
        onClick={() => onChange({ ...config, enabled: !config.enabled })}
        data-testid={`winnerthemeexp-decoration-toggle-${type}`}
      ...
      {config.enabled && (
        <select
          ...
          data-testid={`winnerthemeexp-decoration-intensity-${type}`}
...
function ConfigForm({ initial, onSaved, onCancel }) {
  ...
  const setCountdown = (k, v) => setForm((f) => ({
    ...f,
    appearance: {
      ...f.appearance,
      nextSessionCountdown: { ...DEFAULT_NEXT_SESSION_COUNTDOWN, ...f.appearance.nextSessionCountdown, [k]: v },
    },
  }));
  ...
  const countdown = { ...DEFAULT_NEXT_SESSION_COUNTDOWN, ...form.appearance.nextSessionCountdown };
  ...
  return (
    <div className="rounded-2xl border border-gold/25 p-5 mb-6" style={{ background: "rgba(20,14,32,0.55)" }} data-testid="winnerthemeexp-form">
      <div className="flex items-center gap-2 mb-4">
        <Award className="h-4 w-4 text-gold" />
        <h3 className="font-display text-[15px] text-parchment">
          {initial && initial.id ? `Edit "${initial.key}"` : "New Winner Showcase Theme config"}
        </h3>
        <div className="flex-1" />
        {onCancel && (
          <button onClick={onCancel} data-testid="winnerthemeexp-form-cancel" className="text-faded hover:text-parchment">
            <X className="h-4 w-4" />
          </button>
        )}
      </div>
        ...
        <div>
          {!initial?.id && (
            <Section title="Key (identifies this variant, e.g. 'default' or 'khmer-new-year')">
              <input style={fieldStyle} value={form.key} data-testid="winnerthemeexp-key"
            ...
            <div className="flex gap-2">
              {["followWelcome", "independent"].map((mode) => (
                <button
                  ...
                  data-testid={`winnerthemeexp-syncmode-${mode}`}
          ...
          {form.appearance.syncMode === "independent" && (
            <Section title="Preset">
              <div className="grid grid-cols-3 gap-2" data-testid="winnerthemeexp-preset-grid">
          ...
          <div className="grid grid-cols-2 gap-3">
            {["primary", "secondary", "accent", "scoreColor"].map((k) => (
              <Section key={k} title={k.replace(/([A-Z])/g, " $1")}>
                <input
                  type="color"
                  data-testid={`winnerthemeexp-override-${k}`}
          ...
          <div className="grid grid-cols-2 gap-3">
            <Section title="Style">
              <select
                style={fieldStyle} data-testid="winnerthemeexp-trophy-style"
            ...
            <Section title="Medal design">
              <select
                style={fieldStyle} data-testid="winnerthemeexp-trophy-medal"
            ...
            <Section title="Winner animation">
              <select
                style={fieldStyle} data-testid="winnerthemeexp-trophy-animation"
            ...
            <Section title="Trophy color">
              <input
                type="color" data-testid="winnerthemeexp-trophy-color"
          ...
          <div className="grid grid-cols-2 gap-3">
            <Section title="Shape">
              <select
                style={fieldStyle} data-testid="winnerthemeexp-card-shape"
            ...
            <Section title="Border style">
              <select
                style={fieldStyle} data-testid="winnerthemeexp-card-border"
          ...
          <button type="button"
                  ...
                  data-testid="winnerthemeexp-card-winner-emphasis"
          ...
          <button type="button" onClick={() => setForm((f) => ({ ...f, content: { ...f.content, visible: !f.content.visible } }))}
                  data-testid="winnerthemeexp-visible-toggle"
        ...
        <div>
          ...
          {selectedPreset.seasonalDefault && (
            <button
              ...
              data-testid="winnerthemeexp-use-suggested-dates"
          ...
          <div className="grid grid-cols-2 gap-3">
            <Section title="Starts at (optional)">
              <input type="datetime-local" style={fieldStyle} value={toInputDatetime(form.activeWindow.startsAt)}
                     data-testid="winnerthemeexp-starts-at"
            ...
            <Section title="Ends at (optional)">
              <input type="datetime-local" style={fieldStyle} value={toInputDatetime(form.activeWindow.endsAt)}
                     data-testid="winnerthemeexp-ends-at"
          ...
          <button type="button" onClick={() => setWindow("recurringAnnual", !form.activeWindow.recurringAnnual)}
                  data-testid="winnerthemeexp-recurring-annual"
          ...
          {/* Next Session Countdown — additive, not part of the Achievement
              twin: a live "time until the next Friday classroom session"
              readout on the Dashboard panel. Presentation config, not
              winner data — lives in this theme doc, not the
              winner_showcase snapshot. */}
          <p className="text-[11px] font-bold uppercase tracking-wider text-gold mt-5 mb-2">Next Session Countdown</p>
          <button type="button" onClick={() => setCountdown("enabled", !countdown.enabled)}
                  data-testid="winnerthemeexp-countdown-toggle"
                  className="mb-3 inline-flex items-center gap-2 text-[12px] font-bold text-parchment">
            {countdown.enabled ? <Check className="h-4 w-4 text-green-400" /> : <X className="h-4 w-4 text-faded" />}
            {countdown.enabled ? "Shown on the Dashboard panel" : "Hidden"}
          </button>
          {countdown.enabled && (
            <div className="grid grid-cols-3 gap-3">
              <Section title="Weekday">
                <select
                  style={fieldStyle} data-testid="winnerthemeexp-countdown-weekday"
                  value={countdown.weekday}
                  onChange={(e) => setCountdown("weekday", Number(e.target.value))}
                >
                  {WEEKDAY_LABELS.map((label, idx) => <option key={label} value={idx}>{label}</option>)}
                </select>
              </Section>
              <Section title="Hour (0-23)">
                <input
                  type="number" min={0} max={23} style={fieldStyle} data-testid="winnerthemeexp-countdown-hour"
                  value={countdown.hour}
                  onChange={(e) => setCountdown("hour", Math.max(0, Math.min(23, Number(e.target.value) || 0)))}
                />
              </Section>
              <Section title="Minute (0-59)">
                <input
                  type="number" min={0} max={59} style={fieldStyle} data-testid="winnerthemeexp-countdown-minute"
                  value={countdown.minute}
                  onChange={(e) => setCountdown("minute", Math.max(0, Math.min(59, Number(e.target.value) || 0)))}
                />
              </Section>
            </div>
          )}
          <p className="mt-1.5 text-[10.5px] text-faded">
            The exact classroom time isn't known by this Studio — set it to the real weekly slot.
          </p>
        </div>
      </div>

      {/* Live preview — the ACTUAL FridaySpeakingWinnersPanel, so what's
          shown here is exactly what ships. Shows the real latest
          winner_showcase snapshot (via its own live fetch); only the
          theme layer comes from this draft form. */}
      <div className="mt-5">
        <p className={labelCls}><Eye className="inline h-3 w-3 mr-1" /> Live preview</p>
        <div className="rounded-2xl overflow-hidden p-3" style={{ background: "rgba(0,0,0,0.25)" }} data-testid="winnerthemeexp-preview">
          <FridaySpeakingWinnersPanel themeConfig={previewConfig} />
        </div>
      </div>

      {err && (
        <div className="mt-4 flex items-center gap-2 rounded-lg px-3 py-2 text-[12px]"
             style={{ background: "rgba(255,100,100,0.12)", color: "#fca5a5" }} data-testid="winnerthemeexp-error">
          <AlertTriangle className="h-4 w-4" /> {err}
        </div>
      )}

      <div className="mt-5 flex gap-2">
        <button onClick={handleSubmit} disabled={saving} data-testid="winnerthemeexp-save"
...
function ConfigRow({ c, onEdit, onPublish, onUnpublish, onDuplicate, onDelete }) {
  ...
  return (
    <div className="rounded-xl border border-white/8 p-3 flex items-center gap-3"
         style={{ background: "rgba(30,22,44,0.5)" }} data-testid={`winnerthemeexp-row-${c.id}`}>
      ...
      <button onClick={() => onEdit(c)} data-testid={`winnerthemeexp-edit-${c.id}`} title="Edit" className="text-parchment hover:text-gold">
        <Pencil className="h-4 w-4" />
      </button>
      {c.status === "published" ? (
        <button onClick={() => onUnpublish(c)} data-testid={`winnerthemeexp-unpublish-${c.id}`} title="Unpublish" className="text-parchment hover:text-gold">
          <Undo2 className="h-4 w-4" />
        </button>
      ) : (
        <button onClick={() => onPublish(c)} data-testid={`winnerthemeexp-publish-${c.id}`} title="Publish" className="text-parchment hover:text-green-400">
          <Rocket className="h-4 w-4" />
        </button>
      )}
      <button onClick={() => onDuplicate(c)} data-testid={`winnerthemeexp-duplicate-${c.id}`} title="Duplicate" className="text-parchment hover:text-gold">
        <Copy className="h-4 w-4" />
      </button>
      {confirm ? (
        <button onClick={() => onDelete(c)} data-testid={`winnerthemeexp-delete-confirm-${c.id}`}
        ...
        <button onClick={() => setConfirm(true)} data-testid={`winnerthemeexp-delete-${c.id}`} title="Delete" className="text-red-300/70 hover:text-red-300">
...
export default function WinnerShowcaseThemeStudio() {
  ...
  return (
    <div data-testid="winner-showcase-theme-studio">
      <div className="flex items-center gap-3 mb-5">
        ...
        <div className="flex-1">
          <h2 className="font-display text-xl text-parchment">Winner Showcase Theme</h2>
          <p className="text-[11.5px] text-faded">
            Configure Friday's Speaking Winners panel's theme, decorations, trophy presentation, background artwork,
            seasonal scheduling, and the next-session countdown — the SAME presentation engine as Achievement Experience,
            an independent config so the two can diverge visually over time.
            Until a config is Published here, the panel automatically follows the app's Day/Night theme (Emerald/Midnight Achievement).
          </p>
        </div>
        <button onClick={load} data-testid="winnerthemeexp-refresh"
        ...
        {!showForm && (
          <button onClick={startNew} data-testid="winnerthemeexp-new"
      ...
      {err && (
        <div className="mb-4 flex items-center gap-2 rounded-lg px-3 py-2 text-[12px]"
             style={{ background: "rgba(255,100,100,0.12)", color: "#fca5a5" }} data-testid="winnerthemeexp-list-error">
          <AlertTriangle className="h-4 w-4" /> {err}
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-gold/30 px-6 py-12 text-center"
             style={{ background: "rgba(34,24,48,0.4)" }} data-testid="winnerthemeexp-empty">
          <Award className="mx-auto h-8 w-8 text-gold/70" />
          <p className="mt-3 font-display text-[16px] text-parchment">No Winner Showcase Theme configs yet</p>
          <p className="mt-1 text-[12px] text-faded">The panel is currently following the app's Day/Night theme automatically. Create one to customize it.</p>
        </div>
      ) : (
        <div className="space-y-2.5" data-testid="winnerthemeexp-list">
          {configs.map((c) => (
