 ...
 *
 * Dashboard Polish Round 2, Feature 3 — this FAB used to sit at a
 * hardcoded `bottom-[88px]`, switching to `bottom-5` at the `sm` (640px)
 * breakpoint. MobileBottomNav.jsx only hides at `lg` (1024px,
 * `lg:hidden`), and its own real height varies per device via
 * `--eduhub-bottom-nav-h` (layout-vars.css) plus
 * `env(safe-area-inset-bottom)` — so between 640-1024px wide this FAB
 * dropped to bottom-5 while the nav was still on screen underneath it,
 * and even below 640px the flat 88px guess didn't track the nav's real
 * safe-area-dependent height. Both offsets below now use the SAME
 * shared variable + env() contract layout-vars.css documents for every
 * other fixed/sticky element that must clear the nav, and switch at the
 * same `lg` breakpoint the nav itself uses.
 */
const POP_SHOWN_KEY = "eduhub_tg_pop_shown";
const FAB_BOTTOM_MOBILE = "bottom-[calc(var(--eduhub-bottom-nav-h,64px)_+_env(safe-area-inset-bottom,0px)_+_16px)]";
const FAB_BOTTOM_DESKTOP = "lg:bottom-5";
const POPUP_BOTTOM_MOBILE = "bottom-[calc(var(--eduhub-bottom-nav-h,64px)_+_env(safe-area-inset-bottom,0px)_+_78px)]";
const POPUP_BOTTOM_DESKTOP = "lg:bottom-[80px]";
  ...
  return (
    <>
      <button
        ...
        className={`fixed right-4 ${FAB_BOTTOM_MOBILE} ${FAB_BOTTOM_DESKTOP} z-[500] w-12 h-12 rounded-full
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className={`fixed right-4 ${POPUP_BOTTOM_MOBILE} ${POPUP_BOTTOM_DESKTOP} z-[501]
