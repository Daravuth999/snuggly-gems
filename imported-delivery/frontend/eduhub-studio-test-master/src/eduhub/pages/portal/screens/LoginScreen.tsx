...
import TurnstileWidget, { type TurnstileHandle } from "../../../auth/TurnstileWidget";
...
export function LoginScreen({ onSuccess }: Props) {
  ...
  // v11.0 — Centralized Turnstile widget ref (script loaded globally).
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const tsToken: string = turnstileRef.current ? turnstileRef.current.getToken() : "";
      const result = await loginStudent(id, pw, tsToken);
      if (result?.error) {
      ...
      // AuthContext sets renderStudent state → parent re-renders to dashboard
      if (typeof onSuccess === "function") {
        onSuccess(result as any, 0);
      }
    } catch (err: any) {
      if (turnstileRef.current) turnstileRef.current.reset();
          ...
          <div className="rounded-[28px] border border-[color:var(--color-line)] bg-[color:var(--color-surface)] ink-shadow-lg p-7 sm:p-9">
            ...
            <form onSubmit={handleSubmit} className="space-y-5" data-testid="login-form">
              ...
              {/* v11.0 — Centralized Turnstile widget (script loaded globally
                  from public/index.html with render=explicit). */}
              <TurnstileWidget
                ref={turnstileRef}
                theme="auto"
                size="flexible"
