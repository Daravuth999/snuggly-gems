...
export function PerformanceChart({ history, loading }: Props) {
  ...
  const data = useMemo(
    () =>
      (history || []).map((d, idx) => {
        // v14 (Premium Student PWA Unified UI System v1) — graceful date fallback.
        // Some performance history rows ship without a parsable `date`
        // value (legacy sheet rows, empty string, GAS undefined). The
        // previous label `new Date(d.date).toLocaleDateString(...)`
        // would render literal "Invalid Date" text on the X-axis,
        // visually breaking the chart. We now fall back to "Recent"
        // (EN) / "ថ្មីៗ" (KM), preserving every scoring/aggregation
        // path while removing the broken label. Numeric ordering is
        // unaffected because `idx` is the original row order.
        const raw = d?.date;
        let label;
