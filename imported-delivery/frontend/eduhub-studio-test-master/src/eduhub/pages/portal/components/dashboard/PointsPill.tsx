      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            >
              <defs>
                {/* Aurora: ring stroke uses a band-derived gradient so it
                    matches the rest of the gradient treatment. Start/end
                    stops are the same band colour so behaviour stays
                    identical to the previous flat stroke; the gradient
                    direction (top -> bottom) gives a subtle sheen. */}
                <linearGradient
                  id="pointsRingGradient"
                  x1="0%" y1="0%" x2="0%" y2="100%"
                >
                  <stop offset="0%" stopColor={ringColor} stopOpacity={1} />
                  <stop offset="100%" stopColor={ringColor} stopOpacity={0.85} />
                </linearGradient>
              </defs>
              <circle
                cx={RING_VIEWBOX / 2}
          ...
          {/* ───── Send Points button — Apple-glass premium (v1) ─────
              Visual refresh only; onClick / data-testid / handler
              identity preserved verbatim so the existing P2P modal
              wiring is untouched. */}
          <motion.button
            whileTap={{ scale: 0.96 }}
            ...
            aria-label={t("sendPoints")}
            className="relative h-11 px-4 rounded-full text-sm font-semibold flex items-center gap-2 shrink-0 overflow-hidden group transition-shadow"
            style={{
              // Layered Apple-glass: inner highlight + glass tint +
              // accent gradient + tinted depth shadow.
              background:
                "linear-gradient(180deg, " +
                "color-mix(in oklab, var(--color-accent) 92%, transparent) 0%, " +
                "color-mix(in oklab, var(--color-accent) 78%, transparent) 100%)",
              color: "#fff",
              border:
                "1px solid color-mix(in oklab, var(--color-accent-hover) 55%, transparent)",
              boxShadow:
                "inset 0 1px 0 rgba(255,255,255,0.30), " +
                "inset 0 -1px 0 rgba(0,0,0,0.18), " +
                "0 8px 24px -10px color-mix(in oklab, var(--color-accent) 60%, transparent), " +
                "0 2px 6px -2px rgba(0,0,0,0.45)",
              backdropFilter: "saturate(160%)",
              WebkitBackdropFilter: "saturate(160%)",
            }}
          >
            {/* Top highlight band (gives the glass a subtle "lit-from-above" cue). */}
            <span
              aria-hidden
            ...
            />
            {/* Hover/tap sheen — animates on group:hover, respects reduced motion. */}
            {!reduced && (
              <span
                aria-hidden
            ...
            <span className="relative tracking-wide">{t("send")}</span>
          </motion.button>
        </div>
