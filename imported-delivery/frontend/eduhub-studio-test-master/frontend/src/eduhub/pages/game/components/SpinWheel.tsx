...
export function SpinWheel({
  ...
  // Imperative spin sequence — 3-stage anticipation
  // Stage 1: long deceleration toward (target − segAngle*0.55)
  // Stage 2: slow creep + shudder past to (target + segAngle*0.18)
  // Stage 3: spring back to target
  // This is the proven slot-machine "near-miss" pattern.
  useEffect(() => {
  ...
  // Loading state — slow infinite rotation while preparing the spin
  useEffect(() => {
    if (!isLoading) return;
    ...
    <div className="relative mx-auto aspect-square w-full max-w-[500px]">
      ...
      <div className="absolute inset-[5%] rounded-full border-[6px] border-slate-900 overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.45),inset_0_0_30px_rgba(0,0,0,0.5)]">
        ...
        >
          ...
          {/* Winning-segment spotlight (renders rotated INSIDE the wheel so
              it visually sits on the won slice once the wheel settles). */}
          {!isSpinning &&
            !isLoading &&
            winningIndex !== null &&
...
// so it lines up with the prize slice the user actually won.
function WinSpotlight({
  ...
  // Center angle of the segment in the wheel's local (un-rotated) frame.
  const cx = 50;
  const cy = 50;
