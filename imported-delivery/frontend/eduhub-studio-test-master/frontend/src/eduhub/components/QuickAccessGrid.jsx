    ...
    <div data-testid="quick-access-grid">
      ...
      <div className="grid gap-2.5 sm:gap-3 grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 mb-6">
        {TILES.map((card, i) => {
          ...
          const to = needsAuth && !isAuthenticated
            ? `/login?redirect=${encodeURIComponent(card.to)}`
            : card.to;
          return (
            <motion.div
              key={card.title}
            ...
            >
              ...
              >
                ...
                <div className="relative z-10 flex items-center justify-between mb-3">
                  <div
                    className="w-[40px] h-[40px] rounded-[12px] flex items-center justify-center text-white shadow-[0_6px_22px_rgba(0,0,0,0.45)] transition-transform duration-300 group-hover:scale-110 group-hover:-rotate-6"
                    style={{ background: card.gradient }}
                  >
